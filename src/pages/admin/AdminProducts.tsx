...
const AdminProducts: React.FC = () => {
    ...
    const load = async () => {
        ...
        try {
            ...
            const freshProducts: Product[] = Array.isArray(prodData) ? prodData : prodData?.data || [];
            setProducts(freshProducts);
            setCategories(Array.isArray(catData) ? catData : catData?.data || []);
            // Sync editTarget với data mới nhất (fix checkbox không update sau save)
            setEditTarget(prev => {
                if (!prev) return prev;
                const updated = freshProducts.find(p => p.id === prev.id);
                return updated ?? prev;
            });
        } catch (err: any) {
    ...
    const openEdit = async (p: Product) => {
        ...
        try {
            ...
            setEditTarget(prev => prev?.id === p.id ? { ...prev, ...fresh } : prev);
        } catch {
