        ...
        <div className="min-h-screen bg-[#FAF9F6] p-8 animate-slide-up">
                        ...
                        <table className="w-full text-sm text-left border-collapse">
                            <thead>
                                <tr className="bg-[#FAF7F5] border-b border-[#e8ddd5]/40">
                                    <th className="px-6 py-4 text-[#5c5652] tracking-wider font-bold text-[11px] uppercase tracking-widest w-[15%]">Mã đơn</th>
                                    <th className="px-6 py-4 text-[#5c5652] tracking-wider font-bold text-[11px] uppercase tracking-widest w-[25%]">Khách hàng</th>
                                    <th className="px-6 py-4 text-[#5c5652] tracking-wider font-bold text-[11px] uppercase tracking-widest w-[18%]">Ngày tạo</th>
                                    <th className="px-6 py-4 text-[#5c5652] tracking-wider font-bold text-[11px] uppercase tracking-widest w-[15%]">Tổng tiền</th>
                                    <th className="px-6 py-4 text-[#5c5652] tracking-wider font-bold text-[11px] uppercase tracking-widest w-[17%]">Trạng thái</th>
                                    <th className="px-6 py-4 text-[#5c5652] tracking-wider font-bold text-[11px] uppercase tracking-widest w-[10%] text-center">Thao tác</th>
                                </tr>
                            </thead>
                                ...
                                {paginatedOrders.map((order) => {
                                    ...
                                    return (
                                        <tr key={order.id} className="hover:bg-[#FAF9F6]/50 transition-colors group">
                                            ...
                                            {/* Total Amount */}
                                            <td className="px-6 py-5 text-[#2d2825] font-bold text-sm">
            ...
            {selectedOrder && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#4b2311]/50 backdrop-blur-sm p-4 overflow-y-auto">
                    <div className="bg-white rounded-[32px] border border-[#eaddd2] max-w-3xl w-full overflow-hidden shadow-[0_20px_50px_rgba(75,35,17,0.15)] flex flex-col relative max-h-[90vh] animate-scale-up">
                        ...
                        <div className="flex-1 overflow-y-auto p-8 space-y-6">
                            ...
                            <div className="bg-[#FAF7F5] rounded p-6 border border-[#e8ddd5]/45">
                                <span className="text-[10px] font-bold text-[#68361c]/50 uppercase tracking-widest block mb-4">Trình theo dõi trạng thái</span>
                                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0 relative">
                                    ...
                                    <div className="hidden sm:block absolute left-4 right-4 top-[17px] h-0.5 bg-[#e8ddd5]/70 -z-0"></div>
                                    ...
                                    {ORDER_STATUSES.filter(s => s !== 'Cancelled' && s !== 'Returned').map((status, index) => {
                                        ...
                                        return (
                                            <div key={status} className="flex sm:flex-col items-center gap-3 sm:gap-2 z-10 shrink-0">
                                                <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-black transition-all ${
                                                    isCompleted 
                                                        ? 'bg-[#657b35] text-white shadow-md shadow-[#657b35]/10' 
                                                        : 'bg-white text-stone-400 border-2 border-[#e8ddd5]/80'
                                                } ${isCurrent ? 'ring-4 ring-[#657b35]/15 scale-110' : ''}`}>
                                                    {isCompleted && !isCurrent ? '✓' : `0${index + 1}`}
                                                </div>
                                                <span className={`text-[10px] font-bold uppercase tracking-wider ${isCompleted ? 'text-[#4b2311]' : 'text-stone-400'}`}>
                                                    {statusLabelVi[status]}
                                                </span>
                                            </div>
                                        );
                                ...
                                {selectedOrder.status === 'Cancelled' && (
                                    <div className="mt-4 bg-red-50 border border-red-100 rounded p-3 text-red-700 text-xs font-bold flex items-center gap-2">
                                        ✕ Đơn hàng này đã bị hủy bỏ.
                                    </div>
                                )}
                                {selectedOrder.status === 'Returned' && (
                                    <div className="mt-4 bg-orange-50 border border-orange-100 rounded p-3 text-orange-700 text-xs font-bold flex items-center gap-2">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                                ...
                                <div className="md:col-span-7 space-y-4">
                                    ...
                                    <div className="space-y-3.5 max-h-72 overflow-y-auto pr-1">
                                        {selectedOrder.orderItems?.map((item: any) => (
                                            <div key={item.id} className="flex items-center justify-between gap-4 p-3 bg-white border border-[#e8ddd5]/40 rounded-xl shadow-none">
                                                <div className="min-w-0">
                                                    <h5 className="font-bold text-[#4b2311] text-xs leading-normal truncate" title={item.productName}>
                                                        {item.productName}
                                                    </h5>
                                                    <p className="text-[10px] font-medium text-[#68361c]/60 mt-0.5">
                                                ...
                                                <div className="shrink-0 text-right text-xs">
                                                    ...
                                                    <span className="text-[10px] font-semibold text-[#68361c]/50 block mt-0.5">
                                    ...
                                    {/* Cost breakdown */}
                                    <div className="bg-[#FAF7F5]/50 border border-[#e8ddd5]/30 rounded p-4 text-xs text-[#68361c] space-y-2.5 font-medium">
                                        ...
                                        <div className="flex justify-between items-baseline pt-3 border-t border-[#eaddd2]/40 text-[#4b2311] font-bold">
                                            <span className="uppercase tracking-wider text-[10px]">Tổng thanh toán</span>
                                            <span className="text-base font-black text-[#657b35]">
                                ...
                                <div className="md:col-span-5 space-y-5 text-xs text-[#68361c]">
                                    {/* Shipping details */}
                                    <div className="space-y-3.5">
                                        <h4 className="text-xs font-extrabold text-[#4b2311] uppercase tracking-wider pb-1.5 border-b border-[#eaddd2]/30">
                                            Địa chỉ giao hàng
                                        </h4>
                                        <div className="bg-[#FAF7F5]/30 rounded p-3 border border-[#e8ddd5]/20 space-y-2">
                                            ...
                                            <div className="pt-2 border-t border-[#eaddd2]/20 text-right leading-relaxed font-semibold text-[#68361c]/80">
                                    ...
                                    <div className="space-y-3.5">
                                        ...
                                        <div className="space-y-2 font-medium">
                                            ...
                                            {selectedOrder.note && (
                                                <div className="pt-2">
                                                    <span className="text-[10px] text-[#68361c]/50 uppercase tracking-widest block mb-1">Ghi chú đơn hàng</span>
                                                    <p className="bg-[#FAF7F5] border border-[#e8ddd5]/30 rounded p-2.5 text-[11px] text-[#4b2311] font-semibold leading-relaxed">
                                                        {selectedOrder.note}
                                                    </p>
                                                </div>
                                            )}
                                    ...
                                    <div className="space-y-3 pt-2">
                                        <label className="text-[10px] font-bold text-[#68361c]/50 uppercase tracking-widest block">Thay đổi trạng thái đơn</label>
                                        <div className="flex items-center gap-2">
                                            {ALLOWED_TRANSITIONS[selectedOrder.status]?.length > 0 ? (
                                                <select
                                                    ...
                                                    className="w-full text-xs border border-[#e8ddd5] rounded px-3 py-2 text-[#4b2311] bg-white focus:outline-none focus:border-[#657b35] disabled:opacity-50 font-bold cursor-pointer"
                                                >
                                                ...
                                                <span className="bg-[#FAF6F0] text-[#888079] font-bold px-3 py-2 text-xs rounded border border-[#e8ddd5]/20 block w-full text-center">
                                                    Đơn hàng ở trạng thái cuối (Không thể thay đổi)
                                                </span>
                                            )}
                                            {updating === selectedOrder.id && (
                                                <div className="w-5 h-5 border-2 border-[#657b35] border-t-transparent rounded-full animate-spin shrink-0" />
                                            )}
                                        </div>
                                    </div>
                                </div>
